...
Descriptions of the runtime environment and the winery will be provided in more detail. 
...
The winery provides a complete set of functions for creating, editing and deleting elements in the TOSCA topology. 
...
\caption{TOSCA topology presented by the $Winery$.}
...
Those program components can present not only a complete program but also a certain component of a large application. % or libraries, a packages which can be used only by other programs.
...
The languages used in the framework will be described.
...
Instead of typing commands direct into a command line, a file containing some commands can be executed.~\cite{bash}
This file is called a script.
Scripts are often used to install programs automatically.
