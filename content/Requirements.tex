...
\begin{itemize}
	\item delete references to external packages from a CSAR.
	\item add the internal installation of the packages to the CSAR.
	\item handle different configuration management tools and package managers.
	\item have an easy to expendable structure. 
	\item represent the packages into the TOSCA topology.
	\item generate a new CSAR with the same functionality, but without any external references.
\end{itemize}
